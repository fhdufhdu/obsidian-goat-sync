...
7. `fileCheck` 메시지를 서버에 보낸다.
8. `fileCheckResult upToDate`이면 dirty 항목을 제거한다.
9. `fileCheckResult updateMeta`이면 응답의 서버 메타를 저장하고 dirty 항목을 제거한다.
10. `fileCheckResult put`이면 dirty 항목이 캡처한 `baseVersion`과 현재 로컬 파일 내용으로 `filePut`을 보낸다.
11. `filePutResult okUpdateMeta`이면 응답의 서버 메타를 저장하고 dirty 항목을 제거한다.
12. `fileCheckResult toDeleteLocal` 또는 `filePutResult toDeleteLocal`이면 `selfWriteSuppress`에 delete suppress를 등록하고 로컬 파일을 삭제한 뒤 tombstone 메타를 저장하고 dirty 항목을 제거한다.
13. `conflict`, `deleteConflict`이면 dirty 항목을 제거하고 `blockedPaths`에 등록한다.
14. transient failure가 발생하면 dirty 항목을 남겨 다음 interval 또는 재연결 때 재시도한다.
